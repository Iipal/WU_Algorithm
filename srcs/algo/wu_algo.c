...
/*   Updated: 2019/06/06 16:51:00 by tmaluh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	wu_draw_line(const bool steep, const double_t gradient, double_t intery,
	const __v2du pxl1, const __v2du pxl2, SDL_Surface *restrict const surf) {
...
wu_calc_line(__v2df xy0, __v2df xy1, SDL_Surface *restrict const surf) {
	...
	wu_draw_line(steep, gradient,
		...
		wu_calc_endpoint(xy0, gradient, steep, surf),
		wu_calc_endpoint(xy1, gradient, steep, surf), surf);
}
...
void	wu_algo(Environment *restrict const env) {
	...
	while (env->flags.lines_counter > ++i)
		wu_calc_line(env->line_starts[i], env->line_ends[i], env->sdl.wsurf);
